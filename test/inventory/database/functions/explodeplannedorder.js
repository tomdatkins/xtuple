...
(function () {
  "use strict";
  describe('explodeplannedorder()', function () {
    ...
    it("needs planned order to explode", function (done) {
    ...
    it("should work while exploding children", function (done) {
      var sql  = "select explodePlannedOrder($1, true) as result;",
          cred;
      if (planord && planord.length > 0) {
        cred = _.extend({}, creds, { parameters: [ planord[0].planord_id ]});
        datasource.query(sql, cred, function (err, res) {
          assert.isNull(err);
          assert.equal(res.rows[0].result, planord[0].planord_id);
          done();
        });
      } else {
    ...
    it("should work without exploding children", function (done) {
      var sql  = "select explodePlannedOrder($1, false) as result;",
          cred;
