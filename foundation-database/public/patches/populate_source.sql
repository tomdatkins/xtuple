...
select createDoctype( 3, 'BBI',  'BBI', 'BBI',   'Breeder BOM Item', 'bbomitem', 'bbomitem_id', 'bbomitem_seqnumber::text',   'item_number',   'firstline(item_descrip1)',
                     '', 'join item on bbomitem_item_id = item_id', 'bbomitem_id', 'bbom');
select createDoctype( 4, 'BMH',  'BMH', 'BMH',   'BOM Head',         'bomhead',  'bomhead_id',  'bomhead_docnum',   'item_number',   'firstline(item_descrip1)',
                     '', 'join item on bomhead_item_id = item_id', 'bomhead_id', 'bom');
select createDoctype( 5, 'BMI',  'BMI', 'BMI',   'BOM Item',         'bomitem', 'bomitem_id', 'p.item_number',      'c.item_number', 'firstline(c.item_descrip1)',
                     '', 'join item p on bomitem_parent_item_id = p.item_id join item c on bomitem_item_id = c.item_id');
select createDoctype( 6, 'BOH',  'BOH', 'BOH',   'Routing Head',     'boohead', 'boohead_id', 'boohead_docnum',      'item_number', 'firstline(item_descrip1)',
                     '', 'join item on boohead_item_id = item_id');
select createDoctype( 7, 'BOI',  'BOI', 'BOI',   'Routing Item',     'booitem', 'booitem_id', 'booitem_seqnumber::text',      'item_number', 'firstline(item_descrip1)',
...
select createDoctype(10, 'CNTR', 'CNTR', 'CNTR',   'Contract',         'cntrct',  'cntrct_id',  'cntrct_number',      'vend_name',     'cntrct_descrip',
                     '', 'join vendinfo on cntrct_vend_id = vendid', 'cntrct_id',  'contrct');
select createDoctype(11, 'CM',   'CM', 'CM',    'Return',           'cmhead',  'cmhead_id',  'cmhead_number',      'cust_name',     'firstline(cmhead_comments)',
                     '', 'join custinfo on cmhead_cust_id = cust_id', 'cmhead_id', 'creditMemo');
select createDoctype(12, 'CMI',  'CMI', 'CMI',   'Return Item',      'cmitem',  'cmitem_id',  'cmhead_number',      'cust_name',     'item_number',
...
select createDoctype(17, 'INVI', 'INVI', 'INVI', 'Invoice Item',     'invcitem','invcitem_id','invchead_invcnumber','cust_name', 'item_number', '',
...
select createDoctype(18, 'I',    'I', 'I', 'Item',             'item',    'item_id',    'item_number',         'firstline(item_descrip1)', 'firstline(item_descrip2)',
                     'core', '',     'item_id',    'item');
select createDoctype(19, 'IS',   'IS', 'IS', 'Item Site',        'itemsite','itemsite_id','item_number',         'warehous_code', 'firstline(item_descrip1)',
                     '', 'join item on itemsite_item_id = item_id join whsinfo on itemsite_item_id = warehous_id');
select createDoctype(20, 'IR',   'IR', 'IR', 'Item Source',      'itemsrc', 'itemsrc_id', 'item_number',         'vend_name',     'firstline(item_descrip1)',
                     '', 'join item on itemsrc_item_id = item_id join vendinfo on itemsrc_vend_id = vend_id', 'itemsrc_id', 'itemSource');
select createDoctype(21, 'L',    'L', 'L', 'Location',         'location','location_id','location_formatname', 'warehous_code', 'NULL',
...
select createDoctype(24, 'J',    'J', 'J', 'Project',          'prj',     'prj_id',     'prj_number',          'prj_name',      'firstline(prj_descrip)',
                     'core', '',     'prj_id',     'project',     'MaintainPersonalProjects MaintainAllProjects');
select createDoctype(25, 'P',    'P', 'P', 'Purchase Order',   'pohead',  'pohead_id',  'pohead_number',       'vend_name',     'firstline(pohead_comments)',
                     'core', 'join vendinfo on pohead_vend_id = vend_id', 'pohead_id', 'purchaseOrder');
select createDoctype(26, 'PI',   'PI', 'PI', 'Purchase Order Item', 'poitem',  'poitem_id',  'pohead_number',      'vend_name',     'item_number',
...
select createDoctype(27, 'RA',   'RA', 'RA', 'Return Authorization', 'rahead',  'rahead_id',  'rahead_number',       'cust_name',     'firstline(rahead_notes)',
                     '', 'join custinfo on rahead_cust_id = cust_id', 'rahead_id', 'purchaseOrder');
select createDoctype(28, 'RI',   'RI', 'RI', 'Return Authorization Item', 'raitem',  'raitem_id',  'rahead_number',      'cust_name',     'item_number',
...
select createDoctype(29, 'Q',    'Q', 'Q', 'Quote', 'quhead',  'quhead_id',  'quhead_number',      'cust_name', 'firstline(quhead_ordercomments)',
                     '', 'join custinfo on quhead_cust_id = cust_id', 'quhead_id', 'salesOrder');
select createDoctype(30, 'QI',   'QI', 'QI', 'Quote Item', 'quitem',  'quitem_id',  'quhead_number',      'cust_name',     'item_number',
...
select createDoctype(31, 'S',    'S', 'S', 'Sales Order', 'cohead',  'cohead_id',  'cohead_number',      'cust_name', 'firstline(cohead_ordercomments)',
                     'core', 'join custinfo on cohead_cust_id = cust_id', 'sohead_id', 'salesOrder');
select createDoctype(32, 'SI',   'SI', 'SI', 'Sales Order Item', 'coitem',  'coitem_id',  'cohead_number',      'cust_name',     'item_number',
...
select createDoctype(33, 'SHP',  'SHP', 'SHP', 'Ship To', 'shipto',  'shipto_id',  'shipto_num',         'cust_name',     'shipto_name',
                     '', 'join custinfo on shipto_cust_id=cust_id', 'shipto_id', 'shipTo');
select createDoctype(34, 'TE',   'TE', 'TE', 'Time Expense', 'tehead',  'tehead_id',  'tehead_number',      'emp_number', 'formatDate(tehead_weekending)',
                     '', 'join emp on tehead_emp_id = emp_id', 'tehead_id', 'timeExpenseSheet');
select createDoctype(35, 'TD', 'TODO', 'TODO', 'To-Do',               'todoitem','todoitem_id','todoitem_id::text',  'todoitem_name', 'firstline(todoitem_description)',
                     '', '',     'todoitem_id', 'todoItem', 'MaintainPersonalToDoItems MaintainAllToDoItems');
select createDoctype(36, 'TO',   'TO', 'TO', 'Transfer Order',      'tohead',  'tohead_id',  'tohead_number',      's.warehous_code', 'd.warehous_code',
...
select createDoctype(37, 'TI',   'TI', 'TI', 'Transfer Order Item', 'toitem',  'toitem_id',  'tohead_number',      'item_number',     'warehous_code',
...
select createDoctype(40, 'WH',   'WH', 'WH', 'Site',       'whsinfo', 'warehous_id','warehous_code',         'warehous_descrip', 'NULL');
select createDoctype(41, 'W',    'W', 'W', 'Work Order', 'wo',      'wo_id',      'formatWonumber(wo_id)', 'item_descrip1', 'item_descrip2',
...
select createDoctype(NULL, 'PSPCT',  'PSPCT',   'PSPCT',   'Prospect',     'prospect','prospect_id','prospect_number', 'prospect_name', 'firstline(prospect_comments)');
select createDoctype(NULL, 'SR',     'SR',      'SR',      'Sales Rep',    'salesrep','salesrep_id','salesrep_number', 'salesrep_name', 'NULL');
select createDoctype(NULL, 'TAXAUTH','TAXAUTH', 'TaxAuth', 'Tax Authority','taxauth', 'taxauth_id', 'taxauth_code',    'taxauth_name', 'taxauth_extref');
select createDoctype(NULL, 'USR',    'USR',     'USR',     'User',         'usr',     'usr_id',     'usr_username',    'usr_propername','usr_email');
