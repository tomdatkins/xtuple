-- Function: alterencrypt(text, text)

-- DROP FUNCTION alterencrypt(text, text);

CREATE OR REPLACE FUNCTION alterencrypt(
    text,
    text)
  RETURNS integer AS
$BODY$
  ...
  FOR _vendinfo IN SELECT vend_id, 
      ...
      UPDATE vendinfo
             set vend_ach_routingnumber = encrypt(setbytea(_vendinfo.vend_ach_routingnumber), setbytea(pNewKey), 'bf'),
                 vend_ach_accntnumber = encrypt(setbytea(_vendinfo.vend_ach_accntnumber), setbytea(pNewKey), 'bf')
      WHERE vend_id = _vendinfo.vend_id;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION alterencrypt(text, text)
  OWNER TO admin;