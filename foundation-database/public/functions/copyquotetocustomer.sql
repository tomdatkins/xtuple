...
BEGIN
  ...
  SELECT _quheadid,  quitem_linenumber,  quitem_itemsite_id,
         ...
         CASE WHEN fetchMetricText('WholesalePriceCosting')::boolean THEN (select (item_listcost) from item where item_id=quitem_item_id) ELSE stdCost(itemsite_item_id) END,
         quitem_qty_uom_id,  quitem_price_uom_id,
         quitem_qty_invuomratio,  quitem_price_invuomratio,
