...
BEGIN
  ...
  SELECT recv_qty, recv_date::DATE AS recv_date, recv_freight_curr_id,
    ...
    isControlledItemsite(itemsite_id) AS controlled,
	 (recv_splitfrom_id IS NOT NULL
	   OR (SELECT TRUE
         FROM recv
         WHERE recv_splitfrom_id=recv_id
         LIMIT 1)
    ) AS split INTO _r
  FROM recv 
    ...
    IF (_freight <> 0) THEN
      IF (_r.itemsiteid = -1) THEN
        PERFORM insertGLTransaction( _journalNumber,'S/R', _r.recv_order_type,
          ...
          ROUND(currToBase(_currid, _freight, pEffective), 2),
          pEffective )
        FROM poitem, expcat
        WHERE poitem_expcat_id = expcat_id
          AND poitem_id = _r.recv_orderitem_id
      ...
      ELSEIF (_r.recv_order_type = 'TO' AND fetchMetricBool('MultiWhs')) THEN
        UPDATE toitem
        SET toitem_freight_received = (toitem_freight_received + currToCurr(_currid,
          _o.freight_curr_id, _freight, pEffective))
        WHERE (toitem_id=_r.recv_orderitem_id);
      END IF;
  ...
  ELSE
    -- Receipt not posted yet (NOT _r.recv_posted)
