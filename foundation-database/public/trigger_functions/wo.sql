...
BEGIN
  ...
  ELSIF (TG_OP = 'UPDATE') THEN
    ...
    IF (NEW.wo_qtyord <> OLD.wo_qtyord) THEN
      ...
      PERFORM postComment('ChangeLog', 'W', NEW.wo_id, 'Qty. Ordered',
                          formatQty(OLD.wo_qtyord), formatQty(NEW.wo_qtyord));
    END IF;
    ...
    IF (NEW.wo_duedate <> OLD.wo_duedate) THEN
      ...
      PERFORM postComment('ChangeLog', 'W', NEW.wo_id, 'Due Date',
                          formatDate(OLD.wo_duedate), formatDate(NEW.wo_duedate));
    END IF;
...
DROP TRIGGER IF EXISTS woBeforeDeleteTrigger ON wo;
CREATE TRIGGER woBeforeDeleteTrigger BEFORE DELETE ON wo FOR EACH ROW EXECUTE PROCEDURE _woBeforeDeleteTrigger();
