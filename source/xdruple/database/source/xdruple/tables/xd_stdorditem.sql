...
select xt.add_column('xd_stdorditem','xd_stdorditem_shipto_id', 'integer', '', 'xdruple', 'Shipto database id.');
...
select xt.add_constraint('xd_stdorditem', 'xd_stdorditem_item_id_fkey', 'foreign key (xd_stdorditem_item_id) references item (item_id) on delete cascade', 'xdruple');
select xt.add_constraint('xd_stdorditem', 'xd_stdorditem_cust_id_fkey', 'foreign key (xd_stdorditem_cust_id) references custinfo (cust_id) on delete cascade', 'xdruple');
select xt.add_constraint('xd_stdorditem', 'xd_stdorditem_shipto_id_fkey', 'foreign key (xd_stdorditem_shipto_id) references shiptoinfo (shipto_id) on delete cascade', 'xdruple');
...
-- Cleanup old versions that required shipto_id.
ALTER TABLE xdruple.xd_stdorditem ALTER COLUMN xd_stdorditem_shipto_id DROP NOT NULL;
