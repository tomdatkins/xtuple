...
select xt.add_column('xd_user_contact','xd_user_contact_site_id', 'integer', 'not null references xdruple.xd_site (xd_site_id) on delete cascade', 'xdruple', 'External client application id for this association.');
select xt.add_column('xd_user_contact','xd_user_contact_drupal_user_uuid', 'uuid', null, 'xdruple', 'External client application''s user''s uuid.');
