do $$
  ...
  // Create or update the xdruple package record to accommodate GUI client scripts.
  var version = '1.0.0',
    select = "select pkghead_id from pkghead where pkghead_name = 'xdruple';",
    insert = "insert into pkghead (pkghead_name, pkghead_descrip, pkghead_developer, pkghead_version, pkghead_notes, pkghead_created, pkghead_updated, pkghead_indev) " +
             "values ('xdruple', 'xDruple', 'xtuple', $1, '', now(), now(), false)",
