CREATE OR REPLACE FUNCTION xwd.releaseVendorPr(pVendid INTEGER) RETURNS INTEGER AS $$
...
BEGIN
  ...
  WHERE (itemsrc_id = _pr.itemsrc_id);
  ...
  WHERE ( (charass_target_type='PR')
    AND   (charass_target_id=_pr.pr_id) );

  -- Delete the PR
  PERFORM deletePr(_pr.pr_id);
