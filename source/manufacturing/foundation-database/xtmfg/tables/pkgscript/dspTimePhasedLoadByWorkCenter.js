...
var widgets = toolbox.loadUi("dspTimePhasedLoadByWorkCenter", mywindow);
var layout = toolbox.createLayout("QVBoxLayout", mywindow);
layout.addWidget(widgets);
mywindow.optionsWidget().setLayout(layout);

mywindow.setWindowTitle(qsTr("Time Phased Load by Work Center"));
mywindow.setListLabel(qsTr("Load"));
mywindow.setReportName("TimePhasedLoadByWorkCenter");
...
function setParams(params)
{
  ...
  params.report_name = "TimePhasedLoadByWorkCenter";
...
function sViewLoad()
{
  ...
  var newdlg = toolbox.newDisplay("dspWoOperationsByWorkCenter", 0,
...
function sFillList()
{
  ...
  for (var i = 0; i < selected.length; i++)
  {
    ...
    params.extracolumns += ", xtmfg.loadByWorkCenter(wrkcnt_id, " + cursor.id()
                         + ") AS " + bucketname + ","
                         + " '1' AS " + bucketname + "_xtnumericrole ";

    params.extratoolcolumns += ", (xtmfg.loadByTool(itemsite_id, " + cursor.id()
                         + ")) AS " + bucketname + ","
                         + "  '1' AS " + bucketname + "_xtnumericrole ";
