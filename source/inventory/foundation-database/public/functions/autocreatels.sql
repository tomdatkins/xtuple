...
BEGIN
  ...
  IF (_rows = 0) THEN
    RAISE WARNING 'Itemlocdist record does not exist for id %', pItemlocdistId;
    RETURN -1;
  ELSIF (_r.itemsite_controlmethod NOT IN ('L','S')) THEN
    RAISE WARNING 'Auto create for lot/serial can only be used on lot or serial controlled items';
    RETURN -1;
  ELSIF (_r.itemsite_perishable) THEN
    RAISE WARNING 'Auto create for lot/serial can not be used on perishable items sites';
    RETURN -1;
  ELSIF (_r.itemsite_warrpurc) THEN
    RAISE WARNING 'Auto create for lot/serial can not be used on items sites with purchase warranty';
    RETURN -1;
  ELSIF (_r.itemsite_lsseq_id IS NULL) THEN
    RAISE WARNING 'Auto create for lot/serial requires a lot/serial sequence id';
    RETURN -1;
  END IF;
