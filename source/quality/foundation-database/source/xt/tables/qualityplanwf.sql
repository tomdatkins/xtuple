...
select xt.add_constraint('qualityplanwf', 'qualityplanwf_wfsrc_parent_id_fkey', 'foreign key (wfsrc_parent_id) references xt.qplantype (qplantype_id) on delete cascade');

comment on table xt.qualityplanwf is 'Quality Plan workflow table';

-- TODO migrate workflow form QUALITYPLAN (qphead) to QUALITYPLANTYPE (qplantype)
;
