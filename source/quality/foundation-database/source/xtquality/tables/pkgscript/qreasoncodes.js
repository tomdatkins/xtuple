...
function sNew()
{
  ...
  params.mode         = "new";
  var newdlg          = toolbox.openWindow("qreasoncode", mywindow,
...
function sEdit()
{
  ...
  var params          = new Object;
  params.qtrsncode_id  = _list.id();
  params.mode         = "edit";

  var newdlg          = toolbox.openWindow("qreasoncode", 0,
                                  Qt.ApplicationModal, Qt.Dialog);
  toolbox.lastWindow().set(params);
  newdlg.exec();
...
function popList()
{
  var sql = toolbox.executeQuery("SELECT qtrsncode_id AS id, qtrsncode_code AS name, "
          + "qtrsncode_descrip AS desc FROM xt.qtrsncode");
  _list.populate(sql);  
}  
...
function sDelete()
{
  ...
  var params = new Object;
  params.qtrsncode_id = _list.id();
  var txt = "DELETE FROM xt.qtrsncode WHERE qtrsncode_id = <? value('qtrsncode_id') ?>";
