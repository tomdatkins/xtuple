  ...
  XM.Model = XM.Model.extend(/** @lends XM.Model# */{
    ...
    */
    ...
    doPrint: function (options) {
      ...
      var auxilliaryInfo = this._auxilliaryInfo,
        modelName = this.reportRecordType || this.editableModel || this.recordType,
        reportPayload,
        id = this.id;
      if (!auxilliaryInfo && this.requiresDetail() && this.getValue("detail") && this.getValue("detail").length > 1) {
        return _.each(this.getValue("detail").models, function (det) {
          var printQty = det.get("quantity");
          auxilliaryInfo = JSON.stringify({detail: [{
              quantity: det.get("quantity"),
              trace: det.get("trace"),
              location: det.get("location"),
              expireDate: Globalize.format(det.get("expireDate"), "d")
            }]});
          ...
          reportPayload = {
            ...
            id: id,
            auxilliaryInfo: auxilliaryInfo,
            action: "print",
            printer: options.printer,
            printQty: printQty
          };

          return XT.dataSource.callRoute("generate-report", this.getReportPayload("print", options), {
            success: function () {
    ...
    */
    ...
    getReportUrl: function (action) {
      var modelName = this.reportRecordType || this.editableModel || this.recordType,
        auxilliaryInfo,
        // TODO - move this to the report definition OR forms object/user print pref settings.
        // Only 'print' one document here because it's an email or a pdf in tab.
        printQty = null;
      if (this.requiresDetail() && this.getValue("detail") && this.getValue("detail").length) {
        auxilliaryInfo = JSON.stringify({detail: this.formatDetail()});
        /*auxilliaryInfo = JSON.stringify({
          detail: _.map(this.getValue("detail").models, function (model) {
            return {
      ...
      return XM.Model.getReportUrl(action, modelName, this.id, printQty, auxilliaryInfo);
    },

    getReportPayload: function (action, options) {
      var modelName = this.reportRecordType || this.editableModel || this.recordType,
        auxilliaryInfo = this._auxilliaryInfo,
        /** 
          TODO - move this to the report definition OR forms object/user print pref settings.

          If 'toIssue', 'toReceive', 'toPost' etc. assume that we're printing the label from the
          workspace where the user is in the process of posting the qty. Otherwise, get the
          transacted qty as in printing from a list.
        */
        printQty;
      if (this.printQty) {
        printQty = this.printQty;
      } else if (this.quantityAttribute && this.get(this.quantityAttribute) > 0) {
        printQty = this.get(this.quantityAttribute);
      } else if (this.quantityTransactedAttribute && this.get(this.quantityTransactedAttribute) > 0) {
        printQty = this.get(this.quantityTransactedAttribute);
      } else {
        printQty = null;
      }
      options = options || {}; 
      /*
        If _auxilliaryInfo has not been set through enyo and the model requires detail/has detail,
        then set options.auxilliaryInfo for the generate-report route.

        TODO-refactor ._auxilliaryInfo and below. Choose one way to handle distribution detail obj
      */
      if (!auxilliaryInfo && (this.requiresDetail() && this.getValue("detail") &&
        this.getValue("detail").length)) {
        auxilliaryInfo = JSON.stringify({detail: this.formatDetail()});
        /*auxilliaryInfo = JSON.stringify({
          detail: _.map(this.getValue("detail").models, function (model) {
            return {
