  ...
  XM.Model = XM.Model.extend(/** @lends XM.Model# */{
    ...
    */
    ...
    doPrint: function (options) {
      ...
      var auxilliaryInfo = this._auxilliaryInfo,
        ...
        reportPayload,
        id = this.id;
      if (!auxilliaryInfo && this.requiresDetail() && this.getValue("detail") && this.getValue("detail").length) {
        return _.each(this.getValue("detail").models, function (det) {
          ...
          auxilliaryInfo = JSON.stringify({detail: [{
              ...
              trace: det.get("trace"),
              location: det.getValue("location.id"),
          ...
          return XT.dataSource.callRoute("generate-report", reportPayload, {
            success: function () {
    ...
    */
    ...
    getReportPayload: function (action, options) {
      ...
      } else if (this.quantityAttribute && this.getValue(this.quantityAttribute) > 0) {
        printQty = this.getValue(this.quantityAttribute);
      } else if (this.quantityTransactedAttribute && this.getValue(this.quantityTransactedAttribute) > 0) {
        printQty = this.getValue(this.quantityTransactedAttribute);
      } else {
        printQty = null;
      }
      options = options || {}; 
