  ...
  enyo.kind(
    ...
    captureBarcode: function (inSender, inEvent) {
      ...
      if (matchingModels.length === 1) {
        ...
        */
        if (!_.contains(that.selectedIndexes(), JSON.stringify(index))) {
          // Clear the previous selection(s). Reset the 
          _.each(that.selectedModels(), function (model) {
        ...
        if ((that.getScannedItems()[0] !== index) && (that.getScannedItems().length <= 1)) {
          model.setReqScanAttrs();
        }
        that.setScannedItems([index]);

        // If checkboxes are moved from TransactionList to list_item.js.
        //that.waterfallDown("onScannedItem");

        if (match && scan) {
          // TODO - #2* 
          model.setValue(match, scan);
        }
        // Refresh the line to show the set scanned attributes
    ...
    */
    ...
    setupItem: function (inSender, inEvent) {
      ...
      // XXX - why is model not defined sometimes?
      if (model) {
        /*this.$.itemScanned.setChecked(model.getValue("itemScanned"));
        this.$.traceScanned.setChecked(model.getValue("traceScanned"));
        this.$.locationScanned.setChecked(model.getValue("locationScanned"));*/
      }
    }
  });
