...
(function () {
  ...
  enyo.kind(
    ...
    */
    ...
    setupItem: function (inSender, inEvent) {
      ...
      if (model) {
        var K = XM.ItemSite,
          isLocationControl = model.getValue("itemSite.locationControl"),
          isLotSerialControl = (model.getValue("itemSite.controlMethod") === 
            K.SERIAL_CONTROL) || (model.getValue("itemSite.controlMethod") === K.LOT_CONTROL);

        this.$.itemScanned.setChecked(model.getValue("itemScan") ? true : false);
        // XXX - setShowing would be better here; don't display the checkbox if not required.
        this.$.traceScanned.setDisabled(!isLotSerialControl);
        this.$.traceScanned.setChecked(model.getValue("traceScan") ? true : false);
        this.$.locationScanned.setDisabled(!isLocationControl);
  ...
  // TODO - Replace style with a class, have the class pad them correctly for proper alignment.
  var extensions = [
    {kind: "XV.ListColumn", container: "fittableColumns", addBefore: "listColumn",
      components: [
      {kind: "enyo.Checkbox", name: "itemScanned", style: "display: block; margin-top: 8px; margin-bottom: 8px;"},
      {kind: "enyo.Checkbox", name: "traceScanned", style: "display: block; margin-top: 8px; margin-bottom: 8px;"},
      {kind: "enyo.Checkbox", name: "locationScanned", style: "display: block; margin-top: 8px; margin-bottom: 8px;"}
    ]}
  ];
