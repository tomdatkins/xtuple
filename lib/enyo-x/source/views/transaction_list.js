        ...
        matchingModels = _.filter(items, function (model) {
          ...
          if (isLocControl) {
              if (that.kind === "XV.EnterReceiptList" && that.getScannedItems().length === 1) {
                /**
                  TODO:
                    - define detModels for EnterReceipt as all possible locations for item
                    -
                /*var locations = new XM.LocationCollection(),
                  modelMatch = function () {
                    var isLocation = _.find(locations, function (mod) {
          ...
          if (isLsControl) {
            ...
            if (scanModel ||
              (that.kind === "XV.EnterReceiptList" && that.getScannedItems().length === 1 && model.getValue("locationScan"))) {
              match = "traceScan";
