...
/*global XT:true, XM:true, XV:true, _:true, enyo:true, window: true, console:true, setTimeout:true*/
  ...
  enyo.kind(_.extend(/** @lends XV.List# */{
    ...
    */
    ...
    doDownload: function (options) {
      var that = this,
        model = options.model,
        docType = options.docType || "form", // "default" assume the report is a form, not a label
        rptParams;

      // reportName and customerForm may be defined in the list's print action
      model.reportName = options.action ? options.action.reportName : null;
      model.custFormType = options.action ? options.action.custFormType : null;

      // Define the print function to be called in model.getPrintParameters's callback
      var download = function (params) {
        var reportUrl = "/generate-report?nameSpace=%@&type=%@&id=%@&action=%@".f("ORPT", params.reportName, params.id, "download");
        // Assemble the report params to pass in the url path
        rptParams = "";
        _.each(params.printParameters, function (param) {
          return rptParams += "&param=%@::%@=%@".f(param.name, param.type, param.value);
        });

        that.openReport(XT.getOrganizationPath() + reportUrl + rptParams, function (resp) {
            console.log("openReport success, resp: " + resp);
          }
        );
      };

      if (!model.getPrintParameters) { // Abort
      ...
      model.getPrintParameters(function (params) {
        if (!params || params.error) { // Abort
          return that.doNotify({message: "Error: " + params.error});
        }
    ...
    doEmail: function (options) {
      ...
      var email = function (params) {
        var action = XT.session.config.emailAvailable ? "email" : "display",
          reportUrl = "/generate-report?nameSpace=%@&type=%@&id=%@&action=%@".f("ORPT", params.reportName, params.id, action);
        // Assemble the report params to pass in the url path
        rptParams = "";
        _.each(params.printParameters, function (param) {
          return rptParams += "&param=%@::%@=%@".f(param.name, param.type, param.value);
    ...
    /**
      If a printer is available, print the report silently through the model.
      Otherwise, show the report in another tab.
    */
    doPrint: function (options) {
      var that = this,
        model = options.model,
        docType = options.docType || "form", // default, assume the report is a form, not a label
        printer = XT.defaultPrinter(docType) || null,
        printQty = options.printQty || model.get(model.quantityTransactedAttribute),
        rptParams,
        reportPayload,
      ...
      var print = function (params) {
        ...
        if (printer && printer !== "Browser") {
        ...
        } else if (printer === "Browser") { // Print to Browser
          ...
          // XXX - a callback should be passed into the generate-report route
          // and called after successful print in the route. Somehow.
          if (callback) {
            callback(true);
          }
      ...
      // Get the appropriate report parameters from the model and pass them to the print function
      model.getPrintParameters(function (printParams) {
        if (!printParams || printParams.error) { // Abort
          return that.doNotify({message: "Error: " + printParams.error});
        }

        // No printer, popup a printer picker
        if (!printer) {
          that.doNotify({
            type: XM.Model.QUESTION,
            message: "Select a printer",
            noLabel: "_cancel".loc(),
            component: {kind: "XV.PrinterPicker", showNone: false, label: "_printer".loc()},
            callback: function (resp) {
              // User selected a Printer and tapped OK
              if (resp.answer && resp.componentValue.id) {
                printer = resp.componentValue.id;
                print(printParams);
              } else { // exit doPrint
                return;
              }
        ...
        } else { // printParams obj is defined, proceed to print function.
          print(printParams);
        }
      });
    },
    doPrintForm: function (options) {
      var workspace = options.action.formWorkspaceName,
     ...
     */
    openReport: function (path, callback) {
      var newTab = window.open(path, "_newtab");
      // TODO - the intention was to use this in mocha testing but
      // not able to get content back from "newTab"
      if (callback) {
        setTimeout(function () {
          if (newTab.document.contentType === "application/pdf") {
      ...
      for (prop in this.$) {
        if (this.$.hasOwnProperty(prop)) {
          ...
          if (obj.getAttr) {
            ...
            // XV.ListAttrLabeled kinds
            if (view.$.attr) {
              // TODO - only display the label if there is a value
              //if (value && obj.getLabel && obj.getLabel()) {}
              obj.$.label.addRemoveClass("xv-list-attr", true);
              obj.$.label.setContent(obj.getLabel());
              view.$.attr.setContent(value);
            } else {
