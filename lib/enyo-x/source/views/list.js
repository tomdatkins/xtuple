  ...
  enyo.kind(_.extend(/** @lends XV.List# */{
    ...
    */
    doPrint: function (options) {
      var modelName = (options.model ? (options.model.editableModel || options.model.recordType) : null),
        printer = modelName ? XT.defaultPrinter(modelName) : null;
      if (printer) { // send it to be printed silently by the server
        options.printer = printer;
        options.printQty = options.printQty || options.model.getValue(options.model.quantityTransactedAttribute);
        options.model.doPrint(options);
