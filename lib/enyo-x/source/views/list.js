  ...
  enyo.kind(_.extend(/** @lends XV.List# */{
    ...
    */
    ...
    /**
      ...
      Otherwise, show the report in another tab. On new print actions, "modelName" key is defined.
      On older print actions we can get the "modelName" from the editableModel, i.e. XM.Invoice.
    */
    doPrint: function (options) {
      var model = options.model,
        modelName = model.editableModel || model.recordType,
        printer = modelName ? XT.defaultPrinter(modelName) : null,
        params = "",
        uuid = model.getValue("order") ? model.getValue("order.uuid") : model.getValue("uuid"),
        test;

      /*if (!options.printQty && options.model.quantityTransactedAttribute) {
        options.printQty = model.getValue(model.quantityTransactedAttribute);
      }*/
      if (printer && printer !== "Browser") {
        var reportPayload = {
          nameSpace: "ORPT",
          type: options.action.printReportName,
          param: JSON.stringify(options.action.printParameters), //params
          action: "print",
        ...
        _.each(options.action.printParameters, function (param) {
          if (param.isKey) {
            var dispOptions = {};
            ...
            XM.ModelMixin.dispatch('XM.Model', 'fetchPrimaryKeyId', uuid, dispOptions);
          }
          return params += "&param=%@::%@=%@".f(param.name, param.type, param.value);
        });
        var reportUrl = "/generate-report?nameSpace=%@&type=%@".f(
        "ORPT", options.action.printReportName);

        var callRoute = function () {
          this.openReport(XT.getOrganizationPath() + reportUrl + params);
        }
      }
    },
    /**
