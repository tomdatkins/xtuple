...
/*global XT:true, XM:true, XV:true, _:true, enyo:true, window: true, console:true*/
...
(function () {
  ...
  enyo.kind(_.extend(/** @lends XV.List# */{
    ...
    */
    doPrint: function (options, callback) {
      var that = this,
        model = options.model,
        docType = options.docType || "form", // "default" assume the report is a form, not a label
        printer = XT.defaultPrinter(docType) || null,
      ...
      var print = function (params) {
        ...
        if (printer && printer !== "Browser") {
          ...
          var reportUrl = "/generate-report?nameSpace=%@&type=%@&id=%@&action=%@".f("ORPT", params.reportName, params.id, "display");
          // Assemble the report params to pass in the url path
          rptParams = "";
          _.each(params.printParameters, function (param) {
            return rptParams += "&param=%@::%@=%@".f(param.name, param.type, param.value);
      ...
      model.getPrintParameters(function (printParams) {
        ...
        if (!printer) {
          that.doNotify({
            ...
            callback: function (callbackResp) {
              // User selected a Printer and tapped OK
              if (callbackResp.answer && callbackResp.componentValue.id) {
                printer = callbackResp.componentValue.id;
                print(printParams);
              } else {
                return; // No printer selected or Cancel tapped. Abort.
              }
            }
          });
        } else { // Printer and printParams obj are defined, proceed to print function.
          print(printParams);
        }
      });
    },
    ...
    openReport: function (path, callback) {
      var win = window.open(path, "_newtab");
      // XXX - hack to return 'success' for browser printed routes.
      win.onload = function() {
        console.log("onload");
        console.log("window.document.contentType: " + window.document.contentType);

        callback(window.document.contentType === "application/pdf");
      };
      ...
      window.document.onloadedmetadata = function () {
