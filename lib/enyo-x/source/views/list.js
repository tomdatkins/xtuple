    ...
    */
    doPrint: function (options) {
      var that = this,
        ...
        printer = options.printer || (modelName ? XT.defaultPrinter(modelName) : null),
        routeParams = "";
      // reportName and customerForm may be defined in the list's print action
      if (options.action && options.action.reportName) {
        model.reportName = options.actions.reportName;
      }
      if (options.action && options.action.customerForm) {
        model.customerForm = options.action.customerForm;
      }
