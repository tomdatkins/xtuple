...
(function () {
  ...
  var navigator = /** @lends XV.Navigator# */{
    ...
    */
    ...
    components: [
      ...
      {kind: "FittableRows", name: "list-view", components: [
        ...
        {name: "contentHeader"},
        {name: "emptyListMessage", content: "_emptyList".loc(), style: "font-style: italic", showing: false},
        ...
        {kind: "onyx.Popup", name: "spinnerPopup", showing: false, centered: true, modal: true,
    ...
    backTapped: function (options) {
      ...
      if (index === MODULE_MENU) {
        ...
        this.setMenuPanel(MODULE_MENU);
        if (callback) { // mocha printing test calls backTapped and waits for response
          callback(true);
        }
      }
    },
    ...
    */
    ...
    generateReport: function (inSender, inEvent) {
      var that = this,
        model = inEvent.model || inEvent.kind.getValue(),
        docType = inEvent.docType || "form", // default, assume the report is a form, not a label
        printer = inEvent.printer || XT.defaultPrinter(docType) || null,
        printQty = inEvent.printQty || model.get(model.quantityTransactedAttribute),
        reportAction = inEvent.reportAction,
        rptParams,
        reportPayload,
        callback = inEvent.callback;

      // reportName and customerForm may be defined in the list's print action
      model.reportName = inEvent.action ? inEvent.action.reportName : inEvent.reportName;
      model.custFormType = inEvent.action ? inEvent.action.custFormType : inEvent.custFormType;

      // Define the print function to be called in model.getPrintParameters's callback
      var doReportAction = function (params) {
        // Print to Printer
        if (reportAction === "print" && printer && printer !== "Browser") {
          rptParams = [];
          ...
          XT.dataSource.callRoute("generate-report", reportPayload, {
            success: function (resp) {
              ...
              that.doNotify({message: params.reportName + " #" + params.id + "printed successfully."});
              if (inEvent.success) {
                inEvent.success(true);
              }
              if (XT.session.config.debugging) {
                XT.log(params.reportName + " #" + params.id + "printed successfully.");
              }
            },
            error: function (error) {
              that.doNotify({message: "Print error" + params.reportName + " #" + params.id + ": " + JSON.stringify(error)});
              if (XT.session.config.debugging) {
                XT.log("Print error: " + error);
              }
            }
          });
        } else { // Print to Browser
          var reportUrl = "/generate-report?nameSpace=%@&type=%@&action=%@".f("ORPT", params.reportName, "display");
      ...
      if (!model.getPrintParameters) { // Abort
        return that.doNotify({message: "_modelMissingMethod".loc() + ": getPrintParameters"});
      }
    ...
    */
    fetchedFinished: function (inSender, inEvent) {
      ...
      } else if (!inEvent.isProcessing) {
        this._popupDone = true;
        this.$.spinnerPopup.hide();
        if (inEvent.emptyList) {
