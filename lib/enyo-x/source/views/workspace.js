  ...
  enyo.kind(_.extend({ },
    ...
    */
    print: function (options) {
      var model = options.model || this.getValue(),
        printer = XT.defaultPrinter(model.recordType.suffix());
      if (printer) {
        // send it to be printed silently by the server
        model.doPrint({printer: printer});
      } else if (printer === "Browser" && printer !== "Browser") {
    ...
    save: function (options) {
      ...
      options.success = function (model, resp, options) {
        ...
        if (that.printOnSaveSetting || XT.session.settings.get(that.printOnSaveSetting)) {
